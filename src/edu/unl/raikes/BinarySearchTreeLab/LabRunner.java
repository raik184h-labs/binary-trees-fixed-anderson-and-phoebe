...
public class LabRunner {
    ...
    public static void main(String[] args) {
        ...
        // try to insert Connor to the tree
        insertIntoBSTAndPrintResults(bst, new Person(666666666, "Beth"));

        // try to find the person with NUID 12345678
        searchInBSTAndPrintResults(bst, 12345678);

        // try to delete the person with NUID 12345678
        deleteFromBSTAndPrintResults(bst, 33334444);
    ...
    private static void insertIntoBSTAndPrintResults(BinarySearchTree bst, Person person) {
        boolean isInTree = bst.search(person.key) != null;
        // insert the person
        bst.insert(person);
        // print the results

        if (isInTree) {
            System.out.println("Tried to insert person with NUID " + person.key
                    + "; Person with given NUID is already in the tree");
    ...
    private static void searchInBSTAndPrintResults(BinarySearchTree bst, int key) {
        ...
        // print the results
        if (found == null) {
            System.out.println("Tried to find person with NUID " + key + "; Person does not exist.");
            System.out.println(bst);
        } else {
    ...
    private static void deleteFromBSTAndPrintResults(BinarySearchTree bst, int key) {
        ...
        // print the results
        if (deleted == null) {
            System.out.println("Attempted to delete person with NUID " + key + "; Person does not exist.");
            System.out.println(bst);
        } else {
